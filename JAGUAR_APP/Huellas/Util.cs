...
public class Util
{
    ...
    // Initializes GrFinger ActiveX and all necessary utilities.
    public int InitializeGrFinger(AxGrFingerXLib.AxGrFingerXCtrl grfingerx, dsDetalleHuellas pDetalleHuellas)
    {
        GRConstants result;
        _DB = new DBClass();
    ...
    public int Identify(ref int score)
    {
		...
		{
			// Gettin
         * g current template from recordset.
    ...
    public int StartEnroll()
    {
        ...
        /*_consolidatedTpt = new TTemplate();
        _numberOfConsolidated = 0;

        int ret = 0;// _grfingerx.StartEnroll((int)GRConstants.GR_DEFAULT_CONTEXT);
        return ret;*/
    }
    ...
    public int Consolidate()
    {
        ...
        result = (int)_grfingerx.Enroll(
                                        ref _raw.img, _raw.width, _raw.height, _raw.Res,
                                        ref _consolidatedTpt._tpt, ref _consolidatedTpt._size, 
                                        out qual, (int)GRConstants.GR_FORMAT_DEFAULT,
                                        (int)GRConstants.GR_DEFAULT_CONTEXT);
        // if error, set template size to 0
        if (result < 0)
        {
