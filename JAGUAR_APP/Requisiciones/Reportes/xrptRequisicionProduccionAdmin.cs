...
{
    public partial class xrptRequisicionProduccionAdmin : DevExpress.XtraReports.UI.XtraReport
    {
        Conf_TablesID confTable = new Conf_TablesID();
        public xrptRequisicionProduccionAdmin(int id )
        {
            InitializeComponent();
            LoadHeader(id);
            LoadDetail(id);
        ...
        private void LoadDetail(int id_h)
        {
            try
            {
                ...
                using (SqlConnection cnx = new SqlConnection(dp.ConnectionStringJAGUAR_DB))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("sp_get_requisicion_detalle_V3", con);
                    cmd.CommandType = CommandType.StoredProcedure;
