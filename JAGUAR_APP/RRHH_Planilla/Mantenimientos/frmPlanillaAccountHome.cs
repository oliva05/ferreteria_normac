...
{
    public partial class frmPlanillaAccountHome : DevExpress.XtraEditors.XtraForm
    {
        UserLogin usuarioLog;
        public frmPlanillaAccountHome(UserLogin user)
        {
            InitializeComponent();
            usuarioLog = user;
            CargarDatos();
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            try
            {
                frmPlanillaAccount frm = new frmPlanillaAccount(frmPlanillaAccount.TipoTransaccionPlanilla.Nuevo, usuarioLog);
        ...
        private void btnEditar_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                var row = (dsMantenimientoPlanilla.accountRow)gvAccount.GetFocusedDataRow();

                if (row != null)
                {
                    frmPlanillaAccount frm = new frmPlanillaAccount(frmPlanillaAccount.TipoTransaccionPlanilla.Editar,row.id,usuarioLog);
        ...
        private void btnDelete_ButtonPressed(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                if (respuesta == DialogResult.Yes)
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("dbo.uspDisableAccount", cnx))
                    {
                        cnx.Open();
