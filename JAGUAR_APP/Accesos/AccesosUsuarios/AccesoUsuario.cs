    ...
    public partial class AccesoUsuario : DevExpress.XtraEditors.XtraForm
    {
        ...
        private const string query2 = @"Select T0.id, 
                                              ...
                                              isnull(T4.id,0) [id_conf]
                                                    from [ACS].conf_usuarios T0
                                                    cross join [ACS].conf_sistemas_master_data T1
                                                    cross join [ACS].conf_niveles_acceso T2
                                                    inner join [ACS].conf_sistemas_niveles_acceso T3 on T3.id_nivel = T2.id and T3.id_sistema = T1.id
                                                    left join [ACS].conf_usuarios_niveles_acceso T4 on T4.id_nivel = T3.id and T4.id_user = T0.id
                                                    Where T0.id = @id";

        private const string query3 = @"INSERT INTO [ACS].conf_usuarios_niveles_acceso([id_user],[id_nivel],[enable],[date_start])
                                        VALUES(@id_user, @id_nivel, @enable, @date_start)";

        private const string query4 = @"DELETE FROM [ACS].conf_usuarios_niveles_acceso
                                        WHERE id = @id";
        ...
        DataOperations dp;
        #endregion
        public AccesoUsuario(UserLogin puser)
        {
            InitializeComponent();
        ...
        private void comboSistemas_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (gridViewDetalleSistemas.DataRowCount > 0)
            {
        ...
        private void cmdNuevoUser_Click(object sender, EventArgs e)
        {
            frmUser frm = new frmUser( frmUser.TipoEdicion.Nuevo, UsuarioLogeado,0);
        ...
        private void cmdEditarUsuario_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(comboUsuarios.Text))
            {
                CajaDialogo.Error("DEBE SELECCIONAR UN USUARIO PARA EDITAR");
                return;
            }

            frmUser frm = new frmUser(frmUser.TipoEdicion.Editar, UsuarioLogeado, ((UserData)this.comboUsuarios.EditValue).Id);
