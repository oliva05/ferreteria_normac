...
{
    public partial class frmAddAvanceProduccionBolsasGroupByProducto : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmAddAvanceProduccionBolsasGroupByProducto(UserLogin pUsuarioLogeado, Int64 pIdOrdenFabricacion)
        {
            InitializeComponent();
            CantidadIngresada = 0;
        ...
        public frmAddAvanceProduccionBolsasGroupByProducto(UserLogin pUsuarioLogeado, Int64 pIdOrdenFabricacion, TipoAccion pTipoAccion)
        {
            InitializeComponent();
            ItemSeleccionado = new ItemS();
            UsuarioLogeado = pUsuarioLogeado;
        ...
        private void LoadOrders()
        {
            try
            {
                ...
                //SqlCommand cmd = new SqlCommand("codesahn.sp_get_detalle_production_orders_v3", con);
                SqlCommand cmd = new SqlCommand("[codesahn].[sp_get_detalle_production_orders_v4]", con);
                
                cmd.CommandType = CommandType.StoredProcedure;
                //cmd.Parameters.AddWithValue("@ver_todas", false);
                dsLogisticaJaguar1.Home_Production_ordersGroupByPT.Clear();
                SqlDataAdapter adat = new SqlDataAdapter(cmd);
                adat.Fill(dsLogisticaJaguar1.Home_Production_ordersGroupByPT);
        ...
        private void gridView1_CellValueChanging(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (e.Column.FieldName == "seleccionado")
            {
                ...
                var row = (dsLogisticaJaguar.Home_Production_ordersGroupByPTRow)gridView.GetFocusedDataRow();
                if (Convert.ToBoolean(e.Value))
                {
                    ...
                    ItemSeleccionado.id_pt = row.id_pt;
                    row.seleccionado = true;
                }

                foreach (dsLogisticaJaguar.Home_Production_ordersGroupByPTRow row1 in dsLogisticaJaguar1.Home_Production_ordersGroupByPT)
                {
                    if (row1.id_pt != ItemSeleccionado.id_pt)
        ...
        private void cmdAplicarCambios_Click(object sender, EventArgs e)
        {
            ...
            foreach (dsLogisticaJaguar.Home_Production_ordersGroupByPTRow row1 in dsLogisticaJaguar1.Home_Production_ordersGroupByPT)
            {
                if (row1.seleccionado)
                    ItemSeleccionado.id_pt = row1.id_pt;                     
            }

            if (ItemSeleccionado.id_pt <= 0)
            {
                CajaDialogo.Error("Es necesario seleccionar una orden de fabricación!");
                return;
            }
            ...
            try
            {
                ...
                switch (TipoAccionActual)
                {
                    case TipoAccion.Latas:
                        ...
                        query = "codesahn.[sp_set_insert_avance_produccion_revision_latas_standarv3]";
                        break;
                    case TipoAccion.Bolsas:
                        //query = "codesahn.sp_set_insert_avance_produccion_revision_bolsas_standar";
                        query = "[codesahn].[sp_set_insert_avance_produccion_revision_bolsas_standar_v2]";
                        break;
                }
                ...
                cmd.Parameters.AddWithValue("@id_pt", ItemSeleccionado.id_pt);
