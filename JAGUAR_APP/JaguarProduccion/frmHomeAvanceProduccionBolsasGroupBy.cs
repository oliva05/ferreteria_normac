...
{
    public partial class frmHomeAvanceProduccionBolsasGroupBy : DevExpress.XtraEditors.XtraForm
    {
        UserLogin UsuarioLogeado;
        DataOperations dp;
        public event EventHandler Aplicar;
        public frmHomeAvanceProduccionBolsasGroupBy(UserLogin pUserLogin)
        {
            InitializeComponent();
        ...
        private void LoadDataHome()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("[codesahn].[sp_get_lista_avances_de_orden_fabricacion_bolsas_v3]", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@desde", dtDesde.EditValue);
                cmd.Parameters.AddWithValue("@hasta", dtHasta.EditValue);
                dsProduccion1.avance_produccion_bolsasGroup.Clear();
                SqlDataAdapter adat = new SqlDataAdapter(cmd);
                adat.Fill(dsProduccion1.avance_produccion_bolsasGroup); 
                con.Close();
            }
            catch (Exception ec)
            {
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            frmAddAvanceProduccionBolsasGroupByProducto frm = new frmAddAvanceProduccionBolsasGroupByProducto(this.UsuarioLogeado, 0, frmAddAvanceProduccionBolsasGroupByProducto.TipoAccion.Bolsas);
            frm.EventoPasarId_Lata_inserted += new frmAddAvanceProduccionBolsasGroupByProducto.get_latas_inserted(Ejecutar);
            frm.Show();
        }
